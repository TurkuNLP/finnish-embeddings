...
class Config:
    ...
    save_matrices_to: str = os.getenv("EVAL_DIR")
    ...
    # Evaluation
    top_k: List[int] = field(default_factory=lambda: [1, 5, 10, 25, 50, 100]) # How many articles to retrieve in evaluation
    ...
    def __post_init__(self):
        ...
        if "bm25" in self.model_name:
            if self.test:
                ...
                self.save_results_to: str = f'{os.getenv("TEST_DIR")}/{self.replace_slashes_in_model_name()}/results_test.json'
            else:
                self.save_index_to: str = f'{os.getenv("INDEX_DIR")}/{self.replace_slashes_in_model_name()}/index/'
                self.save_results_to: str = f'{os.getenv("EVAL_DIR")}/{self.replace_slashes_in_model_name()}_results.json'
            return
        ...
        if self.test:
            ...
            self.save_results_to: str = f'{os.getenv("EVAL_DIR")}/{self.replace_slashes_in_model_name()}_resultsIP.json'
